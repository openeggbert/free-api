...
| `LoadStringA` | **Yes — ~90+ call sites, ALL on-screen UI text** | **Yes — ~50+ call sites, ALL on-screen UI text** | eggbert `misc.cpp:36-39` + call sites; planetblupi `misc.cpp:42-45` + call sites | **Must return real, correct string-table text** — tooltips, labels, win/lose text. | **Minimally implemented** — real text via configure-time generated `STRINGTABLE` extraction (`cmake/ExtractStringTable.cmake`; TASK-0074/0075, hardened by TASK-0127/0128) | P0/P1 |
...
| Resource subset | Yes | `FindResourceA`/`LoadResource`/`LockResource`/`SizeofResource`/`FreeResource`/`UnlockResource` all STUB; **this is an acceptable safe stub** since both games' `.rc` files aren't compiled and the games already correctly fall through to file-based loading. **`LoadStringA` is minimally implemented** (real text via configure-time generated `STRINGTABLE` extraction, TASK-0074/0075, hardened by TASK-0127/0128) — both games source ALL on-screen UI text through it. | Real minimal string-table backing for `LoadStringA`. | Low (resolved — see TASK-0074/0075/0127/0128) | P0/P1 (done) |
...
| `LoadStringA` | Resources | **Yes (~90+ sites)** | **Yes (~50+ sites)** | §3.2/§3.8 | Real string-table text for all on-screen UI | **Minimally implemented** (generated `STRINGTABLE` extraction, TASK-0074/0075/0127/0128) | `tests/test_loadstring_regressions.cpp` (done) | P0/P1 (done) |
...
Close behind at the time of the original audit were **`LoadStringA`** (feeds all on-screen UI text in both games) and **`AdjustWindowRect`/`ShowCursor`/`SetCursor`** (client-area sizing and cursor visibility) — all three were `STUB` then, flagged with ⚠ throughout §3/§4/§6, with dedicated investigate-then-fix task pairs in §7 (TASK-0074/0075, TASK-0030/0031, TASK-0051/0052/0053). All three have since been resolved: `LoadStringA` is minimally implemented via configure-time generated `STRINGTABLE` extraction (TASK-0074/0075, hardened further by TASK-0127/0128); `ShowCursor`/`SetCursor` are really implemented via SDL cursor visibility (TASK-0051/0052/0053); `AdjustWindowRect`'s no-op identity transform was investigated and confirmed correct as-is for both games (TASK-0030 DONE, TASK-0031 OBSOLETE — do not "fix" it, see NEXT.md).
