...
namespace {
    ...
    // WM_TIMER, kDiagWmUpdate). FIFO order is preserved by always pushing at
    // the back and popping from the front; coalescing only mutates an
    // existing entry in place or drops a duplicate before it is pushed.
    ...
    // Coalescing of internal update wake-up messages (WM_USER + 1).
    // This message is only an internal redraw/update request generated by the
    // Free API compatibility layer, NOT something the original WinAPI / DDraw
    // platform delivers as a discrete event. Letting it accumulate in the
    // queue produces input lag (real input events queue up behind a backlog
    // of redundant update requests). At most one of these should be pending
    // at any time; PushMessage drops further duplicates while one is queued,
    // and PeekMessageA clears the flag once the message is consumed.
    std::atomic_bool g_updateMessagePending{false};
    // Optional diagnostic counters (only meaningful when diagnostics are on).
