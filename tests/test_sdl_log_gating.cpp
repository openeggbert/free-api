...
static const std::set<std::pair<std::string, int>> kAllowlist = {
    ...
    {"winmm.cpp", 220},           // timeKillEvent: unknown timer id (warning, not fatal)
    {"winmm.cpp", 356},           // mciSendCommandA: avivideo decline (documented, TASK-24H-1111)
    ...
    {"MidiMusic.cpp", 447},       // SDL_InitSubSystem(AUDIO) failed (once-per-process via backendInitFailed latch, TASK-24H-1109)
    {"MidiMusic.cpp", 457},       // SDL_OpenAudioDeviceStream failed (same latch)
    {"MidiMusic.cpp", 604},       // MCI_OPEN: failed to load MIDI file (failure)
