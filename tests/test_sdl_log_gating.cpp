...
static const std::set<std::pair<std::string, int>> kAllowlist = {
    {"winmm.cpp", 144},           // timeSetEvent: invalid args (failure)
    {"winmm.cpp", 160},           // timeSetEvent: SDL_INIT_EVENTS failed (failure)
    {"winmm.cpp", 179},           // timeSetEvent: SDL_AddTimer failed (failure)
    {"winmm.cpp", 219},           // timeKillEvent: unknown timer id (warning, not fatal)
    {"winmm.cpp", 354},           // mciSendCommandA: avivideo decline (documented, TASK-24H-1111)
    ...
    {"MidiMusic.cpp", 471},       // SDL_InitSubSystem(AUDIO) failed (once-per-process via backendInitFailed latch, TASK-24H-1109)
    {"MidiMusic.cpp", 481},       // SDL_OpenAudioDeviceStream failed (same latch)
    {"MidiMusic.cpp", 628},       // MCI_OPEN: failed to load MIDI file (failure)
