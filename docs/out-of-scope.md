...
| `GetTickCount`, `Sleep`, `GetLastError`, `SetLastError`, `RemoveDirectoryA`, `SetEnvironmentVariableA`, `access`/`_access` | Implemented — kept for **test infrastructure**, not proven game-required | None of these seven is called by either game's core source (re-confirmed directly, `TASK-0005`) — they're retained because free-api's own test suite uses them directly (`GetTickCount`/`Sleep`/`RemoveDirectoryA`/`SetEnvironmentVariableA`/`access`/`_access`: `tests/basic_test.cpp`; `SetLastError`: `tests/test_file_regressions.cpp`; `GetLastError`: `tests/basic_test.cpp`). Trivial and harmless either way — this note exists so a future "is this used by the games?" audit isn't confused by their presence. Excludes `CloseHandle`, which looks similar but isn't: it has zero call sites anywhere, including tests — see its own row above (`vestigial`, not test-infrastructure-only). |
